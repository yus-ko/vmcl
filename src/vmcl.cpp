...
//std::vector<pnum> particles; //パーティクル群
//Pose mclPose; //MCLによる推定位置
std::vector<double> measurementLikelihoods(pnum,0); //観測モデルによる算出された各パーティクルの尤度
...
double w = 0.0; //重み
//als::Pose initialNoise(0.5, 0.5, 3.0 * M_PI / 180.0);//初期のパーティクル群の分布
...
std::vector<double> particle_RobotX(pnum,0); //各パーティクルのX座標
std::vector<double> particle_RobotY(pnum,0); //各パーティクルのy座標
std::vector<double> particle_RobotTH(pnum,0); //各パーティクルの角度
std::vector<double> particle_LDM(pnum); //パーティクル・ランドマーク間距離(ユークリッド距離)
std::vector<double> particle_LDMX(pnum); //パーティクル・ランドマーク間X座標誤差
std::vector<double> particle_LDMY(pnum); //パーティクル・ランドマーク間Y座標誤差
std::vector<double> particle_LDMOmega(pnum); //パーティクル・ランドマーク間角度誤差
...
double lutest=0,lutheta=0;

std::vector<double> NoiseV;
std::vector<double> NoiseOmega;

std::vector<double> Est_RobotX(pnum,0);//各パーティクルの世界座標位置
std::vector<double> Est_RobotY(pnum,0);
std::vector<double> Est_RobotTH(pnum,0);
std::vector<double> weight(pnum,1/pnum);//重み

std::vector<double> lu(pnum);//パーティクル-マーカー間距離
std::vector<double> luX(pnum);
std::vector<double> luY(pnum);
std::vector<double> luOmega(pnum);//パーティクル-マーカー間角度


std::vector<double> breez(pnum);//ノイズ付きパーティクル速度
std::vector<double> greed(pnum);//ノイズ付きパーティクル
...
{
	nav_msgs::Odometry encoder_odometry_ = *msg;
	...
	if (markerIds.size() > 0) 
	{
		...
		for(int i=0;i<markerIds.size();i++)
		{
			...
			cv::circle(img_dstfld, cv::Point(MCx[markerIds.at(i)],MCy[markerIds.at(i)]), 3, Scalar(0,255,0),  -1, cv::LINE_AA);//緑点
			...
			depth[markerIds.at(i)] = img_depth.at<float>(cv::Point(pixel[markerIds.at(i)][0],pixel[markerIds.at(i)][1]))+100;
			...
			//std::cout <<"CameraLM["<<markerIds.at(i)<<"][1]="<<CameraLM[markerIds.at(i)][1]*180/M_PI<< std::endl;//マーカーまでの角度
	...
	if (leftLine>0&&rightLine>0&&depth_left_ave>depth_right_ave&&abs(depth_left_ave-depth_right_ave)>1000)//ロボットが右側に寄っているとき
	{
		judge=1;
	}
	else if(leftLine>0&&rightLine>0&&depth_left_ave<depth_right_ave&&abs(depth_left_ave-depth_right_ave)>1000)//ロボットが左側に寄っているとき
	{
		judge=2;
	}
	else
	...
	for (int i=0; i<tate_point_curr.size(); ++i) 
	{
		if(template_size<tate_point_curr[i].y&&tate_point_curr[i].y<480-template_size)
		{
			if(template_size<tate_point_curr[i].x&&tate_point_curr[i].x<640-template_size)
			{
				...
				if(DTPC[i]>0.001&&DTPC[i]<10000&&tate_point_curr[i].x<324.473&&judge==2)	//画像座標上での観測範囲制限（右半分）
				//if(DTPC[i]>0.001&&DTPC[i]<10000&&tate_point_curr[i].x>=324.473)	//画像座標上での観測範囲制限（左半分）
				{
				...
				else if (DTPC[i]>0.001&&DTPC[i]<10000&&tate_point_curr[i].x>=324.473&&judge==1)//画像座標上での観測範囲制限（左半分）
				{
					TPCC[DTPC_ok].x = -DTPC[i] * ((tate_point_curr[i].x - 324.473) / 615.337)/1000;//メートル表示変換
					TPCC[DTPC_ok].y = -DTPC[i] * ((tate_point_curr[i].y - 241.696) / 615.458)/1000;
					TPCC[DTPC_ok].z = DTPC[i]/1000;
				...
				else if (DTPC[i]>0.001&&DTPC[i]<10000&&judge==3)
				{
					TPCC[DTPC_ok].x = -DTPC[i] * ((tate_point_curr[i].x - 324.473) / 615.337)/1000;//メートル表示変換
					TPCC[DTPC_ok].y = -DTPC[i] * ((tate_point_curr[i].y - 241.696) / 615.458)/1000;
					TPCC[DTPC_ok].z = DTPC[i]/1000;
	...
	//DTPC_okkk<< DTPC_ok<<"\n";
	tate_point_curr.resize(DTPC_ok);//Depth取得数//でリサイズ(tate_point_camera_c)
	...
	if(time0 != false)
	{
		if (reset == false)	//初回以降動作
		{
			...
			for (int i = 0; i < matchT_curr; i++) 
			{
				...
				if(DMT_curr[i]>0.001&&DMT_curr[i]<10000&&MTPC[i].x<324.473&&judge==2)
				{
				...
				else if(DMT_curr[i]>0.001&&DMT_curr[i]<10000&&MTPC[i].x>=324.473&&judge==1)
				{
					MT_curr_Templ[DMT_curr_ok] = MTTC[i];//Depth取得可能なマッチテンプレート
					MT_curr_pixel[DMT_curr_ok] = MTPC[i];//Depth取得可能なマッチング中心画像座標
					MT_curr_camera[DMT_curr_ok].x = -DMT_curr[i] * ((MTPC[i].x - 324.473) / 615.337)/1000;//カメラ座標変換
				...
				else if(DMT_curr[i]>0.001&&DMT_curr[i]<10000&&judge==3)
				{
					MT_curr_Templ[DMT_curr_ok] = MTTC[i];//Depth取得可能なマッチテンプレート
					MT_curr_pixel[DMT_curr_ok] = MTPC[i];//Depth取得可能なマッチング中心画像座標
					MT_curr_camera[DMT_curr_ok].x = -DMT_curr[i] * ((MTPC[i].x - 324.473) / 615.337)/1000;//カメラ座標変換
	...
	if(time0 != false)
	{
		...
		for(int v=0;v<pnum;v++)
		{
			if(greed[v]<1e-10)
			{
				Est_RobotX[v]=Est_RobotX[v]+(realsec*breez[v]*cos(Est_RobotTH[v]));
				Est_RobotY[v]=Est_RobotY[v]+(realsec*breez[v]*sin(Est_RobotTH[v]));
				Est_RobotTH[v]=Est_RobotTH[v]+(greed[v]*realsec);
				//std::cout << "Est_RobotTH[v]" <<Est_RobotTH[v]<< std::endl;
				aggrex=aggrex+Est_RobotX[v];
				aggrey=aggrey+Est_RobotY[v];
				aggrez=aggrez+Est_RobotTH[v];
			}
			else
			{
				Est_RobotX[v]=Est_RobotX[v]+((breez[v]/greed[v])*(sin(Est_RobotTH[v]+greed[v]*realsec)-sin(Est_RobotTH[v])));
				Est_RobotY[v]=Est_RobotY[v]+((breez[v]/greed[v])*(-cos(Est_RobotTH[v]+greed[v]*realsec)+cos(Est_RobotTH[v])));
				Est_RobotTH[v]=Est_RobotTH[v]+(greed[v]*realsec);
				aggrex=aggrex+Est_RobotX[v];
				aggrey=aggrey+Est_RobotY[v];
				aggrez=aggrez+Est_RobotTH[v];
		...
		for (int j = 0; j < markerIds.size(); j++)
		{
			for (int i = 0; i < pnum; i++)
			{ 
				lu[i]=sqrt((Est_RobotX[i]-MarkerW[markerIds.at(j)](2,0))*(Est_RobotX[i]-MarkerW[markerIds.at(j)](2,0))+(Est_RobotY[i]-MarkerW[markerIds.at(j)](0,0))*(Est_RobotY[i]-MarkerW[markerIds.at(j)](0,0)));

				luX[i]=Est_RobotX[i]-MarkerW[markerIds.at(j)](2,0);//平均値のX成分（山口先輩）
				//luX[i]=MarkerW[markerIds.at(j)](2,0)-Est_RobotX[i];//平均値のX成分（鈴木先輩）      
				luY[i]=Est_RobotY[i]-MarkerW[markerIds.at(j)](0,0);//平均値のY成分
				luOmega[i]=atan2(luY[i],luX[i]);
				
				if (luOmega[i]>0)
				{
					luOmega[i]=luOmega[i]-2*3.1415;
				}
				lutest=lutest+lu[i];
				lutheta=lutheta+(Est_RobotTH[i]+1.57)-abs(luOmega[i]);
			}
			double xxx;
			double yyy;
			double kankyori;
			...
			for (int i = 0; i < pnum; i++)
			{
				kankyori=abs(abs(CameraLM[markerIds.at(j)][0])-abs(lu[i]));//(山口先輩)

				//yyy=0.000856;//角度分散（鈴木先輩）
				yyy=0.000464;//(山口先輩)

				xxx=lu[i]*lu[i]*0.000856;//距離分散
				
				//↓変曲点に着目した距離分散変動
				if (abs(abs(CameraLM[markerIds.at(j)][0])-abs(lu[i]))>0.5&&abs(abs(CameraLM[markerIds.at(j)][0])-abs(lu[i]))<1.0)
				{
					xxx=abs(abs(CameraLM[markerIds.at(j)][0])-abs(lu[i]))*abs(abs(CameraLM[markerIds.at(j)][0])-abs(lu[i]));
				}

				double kakurituX=1/(sqrt(2*3.1415*xxx))*exp(-((abs(CameraLM[markerIds.at(j)][0])-abs(lu[i]))*(abs(CameraLM[markerIds.at(j)][0])-abs(lu[i])))/(2*xxx))+1e-100;//ランドマークまでの距離
				
				double kakurituY=1/(sqrt(2*3.1415*yyy))*exp(-((CameraLM[markerIds.at(j)][1]-((Est_RobotTH[i]+1.57)-abs(luOmega[i])))*(CameraLM[markerIds.at(j)][1]-((Est_RobotTH[i]+1.57)-abs(luOmega[i]))))/(2*yyy))+1e-100;//（山口先輩）
				...
				if (luOmega[i]*Est_RobotTH[i]>0&&luOmega[i]>1.57)
				{
					kakurituY=1/(sqrt(2*3.1415*yyy))*exp(-((CameraLM[markerIds.at(j)][1]-(-luOmega[i]-(Est_RobotTH[i]-2*3.1415)))*(CameraLM[markerIds.at(j)][1]-(-luOmega[i]-(Est_RobotTH[i]-2*3.1415))))/(2*yyy))+1e-100;//
				}
				else if (luOmega[i]*Est_RobotTH[i]>0&&Est_RobotTH[i]<-1.57)
				{
					kakurituY=1/(sqrt(2*3.1415*yyy))*exp(-((CameraLM[markerIds.at(j)][1]-(-luOmega[i]-(Est_RobotTH[i]+2*3.1415)))*(CameraLM[markerIds.at(j)][1]-(-luOmega[i]-(Est_RobotTH[i]+2*3.1415))))/(2*yyy))+1e-100;//
				}
				else
				...
				//↓0415変曲点改良案
				if (abs(abs(CameraLM[markerIds.at(j)][0])-abs(lu[i]))>1.0)
				{
					kakurituX=1;
				}
				
				weight[i]*=(kakurituX*kakurituY);
				if (weight[i]==0)
				{
			...
			for(int i = 0; i < pnum; i++)
			{
				if(i == 0)
				{
					maxLikelihood = weight[i];
					maxLikelihoodParticleIdx = 0;
				}
				else if (maxLikelihood < weight[i])
				{
					maxLikelihood = weight[i];
					maxLikelihoodParticleIdx = i;
				}
				totalLikelihood += weight[i];
		...
		for (int i = 0; i < pnum; i++)
		{
			if (weight[i] < 1e-100)
			{
				weight[i] = weight[i] + 1e-100;
			}
			
			ss.push_back(weight[i]);
		}
		...
		int cur_pos=0;
		int math=0;
		std::vector<double> ps1(pnum,0);//新たに抽出するEstXのリスト
		std::vector<double> ps2(pnum,0);//新たに抽出するEstYのリスト
		std::vector<double> ps3(pnum,0);//新たに抽出するEstTHのリスト

		while (math < pnum)//もとのパーティクル数と一致するまで
		{
			if (r < sd[cur_pos])//重みが幅より大きい場合
			{
				ps1[math]=Est_RobotX[cur_pos];
				ps2[math]=Est_RobotY[cur_pos];
				ps3[math]=Est_RobotTH[cur_pos];
				math=math+1;
				r+=step;
			}
			else
		...
		copy(ps1.begin(),ps1.end(),Est_RobotX.begin());
		copy(ps2.begin(),ps2.end(),Est_RobotY.begin());
		copy(ps3.begin(),ps3.end(),Est_RobotTH.begin());
	...
	particles.header.stamp = ros::Time::now();//追加
	particles.header.frame_id = source_frame;//追加
	for(int v = 0; v < pnum; v++)
	{
		geometry_msgs::Pose pose;
		pose.position.x = Est_RobotX[v];
		pose.position.y = Est_RobotY[v];
		pose.position.z = 0;
		pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(0,0,Est_RobotTH[v]);
...
int main(int argc,char **argv)
{
	...
	message_filters::Subscriber<nav_msgs::Odometry> odom_sub(nhSub, "/robot1/odom", 1);//センサーメッセージを使うときは対応したヘッダーが必要
  	//robot/odomからnav_msgs::Odometry型の情報が送り込まれる（コールバック関数）
